...
async function exportAllJson(){
  ...
  const payload = { exportedAt:new Date().toISOString(), library:LIBRARY, versionsIndex:VERSIONS_INDEX, versions };
...
async function init(){
  ...
  window.supabase.auth.onAuthStateChange((event, session)=>{
    ...
    if(event === 'SIGNED_IN' && session){ clearLibraryHistory(); clearVersionHistory(); loadAuthedAppState(); }
  });
